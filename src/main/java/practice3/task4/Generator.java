...
public class Generator implements ObservableOnSubscribe<File> {
    ...
    public Generator(String[] arrTypes, int MIN_SIZE, int MAX_SIZE, int MIN_PERIOD, int MAX_PERIOD) {
    ...
    private void generateSchedule(@NonNull ObservableEmitter<File> emitter) {
        Timer timer = new Timer();
        TimerTask task = new TimerTask() {
            public void run()
            {
                emitter.onNext(new File(generateType(), generateSize()));
                generateSchedule(emitter);
            }
        };
